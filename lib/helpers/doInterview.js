    ...
    if (channel) {
      ...
      models.Standup.findAll({
        ...
        if (standups.length > 0) {
          ...
          oldTodo += standups[standups.length-1].today + ' \n';
          oldGoal += 'Last time, you said your goal was:\n>';
          oldGoal += standups[standups.length-1].goal + ' \n';

          // only want the last few blockers. not all.
          standups.slice(-3, standups.length).forEach(function(stup){
            pastBlockers += stup.blocker + '\n'
          })
