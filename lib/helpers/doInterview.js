    ...
    if (channel) {
      ...
      models.Standup.findAll({
        ...
        if (standups.length > 0) {
          ...
          oldTodo += 'Last time, you said that you would:\n> ';
          oldTodo += standups.slice(-1)[0].today + ' \n';
          oldGoal += 'Last time, you said your goal was:\n>';
          oldGoal += standups.slice(-1)[0].goal + ' \n';

          // only want the last few blockers. not all.
          standups.slice(-3).forEach(function(standup){
            if (standup.blocker != undefined) {
              pastBlockers += standup.blocker + '\n'
            }
          })
        ...
        bot.startPrivateConversation({user: interviewUser}, function(response, convo){
          ...
          interviewSetup.then(function() {
            ...
            const sections = [
              {
                question: `${oldTodo} What did you do yesterday? `,
                name: 'yesterday'
              },
              {
              ...
              {
                question: `${oldGoal} Any major goal for the day?`,
                name: 'goal'
              }
            ];
              ...
              if(!singleSection || singleSection == section.name) {
                convo.ask(section.question, function(response, conversation) {
                  if(!checkForExit(response, conversation)) {
                    if (!response.text.match((/^skip|^none|^nope|^nothing/i))) {
